...
export interface RoomInfoPanelProps {
  ...
  // テーブル数編集
  editingTableCount: boolean;
  setEditingTableCount: (v: boolean) => void;
  tableCountInput: number;
  setTableCountInput: (v: number) => void;
...
export default function RoomInfoPanel({
  ...
  const handlePrintQR = () => {
    ...
    // SVGをdata URIに変換
    const svgEl = document.querySelector(".bg-white.p-3 svg");
    let svgDataUri = "";
    if (svgEl) {
      ...
      <div className="space-y-4">
          ...
          {editingTableCount ? (
            <div className="flex gap-2 items-center">
              <input
                ...
                min={1}
                max={20}
                className="w-20 px-2 py-1 bg-gray-800 border border-gray-700 rounded focus:outline-none focus:border-blue-500"
              />
              <button
              ...
              <button
                onClick={() => {
                  ...
                  setTableCountInput(room.config.tableCount);
                }}
                className="px-2 py-1 bg-gray-700 hover:bg-gray-600 rounded text-sm transition"
              >
                取消
              </button>
            </div>
            ...
            <p
              ...
              onClick={() => setEditingTableCount(true)}
            >
              {room.config.tableCount}卓
              <span className="text-xs text-gray-500 ml-2">（変更）</span>
            </p>
          )}
        </div>
        ...
        <div>
          <div className="flex items-center justify-between mb-2">
            <p className="text-sm text-gray-400">エントリー項目</p>
